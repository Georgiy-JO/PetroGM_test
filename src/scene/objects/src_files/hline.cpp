...
namespace scene{
    ...
    void HLine::SetPos(double x1, double x2, double y){
        if (x1 > x2) m_beg.x = x2, m_end.x = x1;
        else m_beg.x = x1, m_end.x = x2;
        m_beg.y = m_end.y = y;
    }
    Vec2 HLine::GetBeg() const{return m_beg;}
    Vec2 HLine::GetEnd() const{return m_end;}
    void HLine::Draw(uint8_t* buffer, unsigned int width_in_pixels, unsigned int hight_in_pixels,
        const Pixel& pixel_size, const Vec2& scene_beg, const Vec2& scene_end) {
