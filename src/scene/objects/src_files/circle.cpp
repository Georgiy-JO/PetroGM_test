...
namespace scene {
  ...
  Circle::Circle(const Vec2 &center, double rr):m_center(center){
    SetR(rr);
  }
  Circle::Circle(const Circle &other):m_center(other.m_center),m_r(other.m_r){

  }
  void Circle::SetCenter(const Vec2 &center){
    m_center=center;
  }
  void Circle::SetR(double rr){
    if(rr<0)
        throw std::runtime_error("Circle Setting Error:Radius can't be negative.");
    m_r=rr;
  }
  ...
  void Circle::Draw(uint8_t *buffer, unsigned int width_in_pixels,
        ...
        if (IsThereParametersIssue(buffer, width_in_pixels, hight_in_pixels, pixel_size) ||
            (m_center.x+m_r<scene_beg.x)||(m_center.x-m_r>scene_end.x)||
            (m_center.y+m_r<scene_beg.y)||(m_center.y-m_r>scene_end.y)){ return;}
        ...
        for (unsigned int y = y_min_pix; y <= y_max_pix; y++) {
            for (unsigned int x = x_min_pix; x <= x_max_pix; x++) {
                ...
                squared_pixel_delta = std::pow((pixel_center.x - center_in_scene_coord.x),2) + std::pow((pixel_center.y - center_in_scene_coord.y),2);
