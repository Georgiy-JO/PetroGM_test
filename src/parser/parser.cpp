...
namespace parser{
    ...
    std::unique_ptr<Object> Parser::CreateObjectFromLine(const std::string& line) const{
        ...
        //     float x, y, r; iss >> x >> y >> r; return std::make_unique<Circle>(x, y, r);
        // } else if (type == "triangle") {  // Custom type (outline for now)
        //     float x1, y1, x2, y2, x3, y3; iss >> x1 >> y1 >> x2 >> y2 >> x3 >> y3;
        //     return std::make_unique<Triangle>(x1, y1, x2, y2, x3, y3);
        }
        return nullptr;
    }
}   // namespace parser